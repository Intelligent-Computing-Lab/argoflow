...
func (woc *wfOperationCtx) getWorkflowDeadline() *time.Time {
	...
	log.Trace("++++++++++++++++++++++姜猛修改标记，输出一下getWorkflowDeadline()方法内的startedAt的值为:" + startedAt.String() + " 输出完毕+++++++++++++++++++++++++")
	log.Trace("++++++++++++++++++++++姜猛修改标记，输出一下getWorkflowDeadline()方法内的给StartedAt增加的时间的值为:" + (time.Duration(*woc.execWf.Spec.ActiveDeadlineSeconds) * time.Second).String() + " 输出完毕+++++++++++++++++++++++++")
	s := strconv.FormatInt(*woc.execWf.Spec.ActiveDeadlineSeconds, 10)
	log.Trace("++++++++++++++++++++++姜猛修改标记，输出一下time.Duration的参数值用的是: " + s + " 输出完毕+++++++++++++++++++++++++")

	log.Trace("++++++++++++++++++++++姜猛修改标记，输出一下getWorkflowDeadline()方法内的deadline的值为:" + deadline.String() + " 输出完毕+++++++++++++++++++++++++")
...
func (woc *wfOperationCtx) executeTemplate(ctx context.Context, nodeName string, orgTmpl wfv1.TemplateReferenceHolder, tmplCtx *templateresolution.Context, args wfv1.Arguments, opts *executeTemplateOpts) (*wfv1.NodeStatus, error) {
	log.Trace("++++++++++姜猛修改标记，开始调用executeTemplate方法，推测是开始创建node+++++++++++")
...
func (woc *wfOperationCtx) executeData(ctx context.Context, nodeName string, templateScope string, tmpl *wfv1.Template, orgTmpl wfv1.TemplateReferenceHolder, opts *executeTemplateOpts) (*wfv1.NodeStatus, error) {
	node := woc.wf.GetNodeByName(nodeName)
	log.Trace("++++++++++++姜猛修改标记，开始调用executeData方法，这里打印node的name为：" + node.Name + "   FinishedAt为：" + node.FinishedAt.String() + "打印结束++++++")
	...
	log.Trace("+++++++++++姜猛修改标记，此处开始调用createWorkflowPod方法，来创建pod++++++++++++++++")
