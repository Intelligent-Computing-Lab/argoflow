...
func (wfc *WorkflowController) Run(ctx context.Context, wfWorkers, workflowTTLWorkers, podCleanupWorkers int) {
	log.Trace("++++++++++++++++++++++姜猛修改标记，开始调用controller.go的Run方法了，此方法内有直接获取action内容的步骤")
	...
	for i := 0; i < podCleanupWorkers; i++ {
		log.Trace("+++++++++++++++姜猛修改标记，不停循环cleanupworkers队列，执行接受action++++++++++++++++++++++")
	...
	log.Trace("+++++++++++++++++++++++++++++++姜猛修改标记，开始runGCcontroller，开启垃圾回收队列的工作+++++++++++++++++++++++++++")
	...
	for i := 0; i < wfWorkers; i++ {
		log.Trace("+++++++++++++++姜猛修改标记，不停循环wfWorkers队列，执行准备创建node的runWorker方法++++++++++++++++++++++")
...
func (wfc *WorkflowController) processNextPodCleanupItem(ctx context.Context) bool {
	...
	log.Trace("+++++++++++++姜猛修改标记，收到的action是：" + action + " 打印完毕++++++++++++++++++++++++++")
	...
	err := func() error {
		...
		case labelPodCompleted:
			...
			log.Trace("++++++++++++姜猛修改标记，调用pod.patch完毕+++++++++++++++++++")
